...
class FinerLayer(nn.Module):
    def __init__(self, in_features, out_features, bias=True, is_first=True, omega_0=30, first_bias_scale=None, scale_req_grad=False):
        super().__init__()
        self.omega_0 = omega_0
        self.is_first = is_first
        self.in_features = in_features
    ...
    def init_first_bias(self):
            with torch.no_grad():
                if self.is_first and self.linear.bias is not None:
...
class GeluLayer(nn.Module):
    ...
    def init_weights(self):
        with torch.no_grad():
            ...
            bound = np.sqrt(6 / self.in_features)
...
class GeluNet(nn.Module):
    def __init__(self, in_features, hidden_features, hidden_layers, out_features, bias=True):
        super().__init__()

        layers = []

        # First layer (no special handling)
        layers.append(GeluLayer(in_features, hidden_features, bias=bias))

        # Hidden layers (same structure & init)
        for _ in range(hidden_layers):
            layers.append(GeluLayer(hidden_features, hidden_features, bias=bias))

        # Final linear (same initialization rule)
...
class SineLayer_1(nn.Module):
    ...
    def generate_scale(self, x):
        if self.scale_req_grad: 
            scale = torch.sqrt(1 + x**2) + torch.abs(x)
        else:
            with torch.no_grad():
                scale = torch.sqrt(1 + x**2) + torch.abs(x)
        return scale
...
class Siren_1(nn.Module):
    def __init__(self, in_features, hidden_features, hidden_layers, out_features, first_omega_0=30, hidden_omega_0=30.0, bias=True, first_bias_scale=None, scale_req_grad=False):
        ...
        self.net = []
        self.net.append(SineLayer_1(in_features, hidden_features, is_first=True, omega_0=first_omega_0, bias=bias,first_bias_scale=first_bias_scale, scale_req_grad=scale_req_grad))

        for i in range(hidden_layers):
            self.net.append(SineLayer_1(hidden_features, hidden_features, is_first=False, omega_0=hidden_omega_0,scale_req_grad=scale_req_grad))
